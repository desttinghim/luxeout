...
Void App_obj::on_internal_init( ){
{
		...
		HX_STACK_THIS(this)
		HX_STACK_LINE(110)
		this->cur_frame_start = ::snow::Snow_obj::core->timestamp();
		HX_STACK_LINE(111)
...
Void App_obj::on_internal_update( ){
{
		...
		if (((this->update_rate != (int)0))){
			HX_STACK_LINE(121)
			if (((this->next_tick < ::snow::Snow_obj::core->timestamp()))){
				HX_STACK_LINE(123)
				this->next_tick = (::snow::Snow_obj::core->timestamp() + this->update_rate);
			}
			else{
				HX_STACK_LINE(126)
				return null();
		...
		if (((this->render_rate != (int)0))){
			HX_STACK_LINE(151)
			if (((this->next_render < ::snow::Snow_obj::core->timestamp()))){
				HX_STACK_LINE(152)
				this->app->render();
				HX_STACK_LINE(153)
