	...
	HX_STACK_ARG(_strict,"_strict")
{
		...
		HX_STACK_LINE(33)
		bool on_disk = ::sys::FileSystem_obj::sys_exists(::haxe::io::Path_obj::removeTrailingSlashes(_path));		HX_STACK_VAR(on_disk,"on_disk");
		HX_STACK_LINE(35)
	...
	HX_STACK_ARG(_onload,"_onload")
{
		...
		if (((_native_info != null()))){
			struct _Function_2_1{
				inline static Dynamic Block( Dynamic &_native_info){
					HX_STACK_FRAME("*","closure",0x5bdab937,"*.closure","snow/platform/native/assets/AssetSystem.hx",48,0xc6d1b123)
					{
						...
						__result->Add(HX_CSTRING("data") , ::snow::platform::native::utils::UInt8Array_obj::__new(_native_info->__Field(HX_CSTRING("data"),true),null(),null()),false);
						return __result;
					}
					return null();
				}
			...
			HX_STACK_LINE(48)
			info = _Function_2_1::Block(_native_info);
		}
		HX_STACK_LINE(62)
		if (((_onload != null()))){
			HX_STACK_LINE(63)
	...
	HX_STACK_ARG(_components,"_components")
{
		...
		HX_STACK_LINE(78)
		Dynamic info = _Function_1_1::Block(_native_info);		HX_STACK_VAR(info,"info");
		HX_STACK_LINE(89)
		_native_info = null();
		HX_STACK_LINE(91)
		return info;
...
Dynamic AssetSystem_obj::audio_load_info( ::String _path,int _format,Dynamic __o__load,Dynamic _onload){
	...
	HX_STACK_ARG(_onload,"_onload")
{
		...
		switch( (int)(_format)){
			case (int)2: {
				HX_STACK_LINE(104)
				info = this->audio_load_wav(_path,_load);
			}
			;break;
			case (int)1: {
				HX_STACK_LINE(108)
				info = this->audio_load_ogg(_path,_load);
			}
			;break;
			case (int)3: {
				HX_STACK_LINE(112)
				info = this->audio_load_pcm(_path,_load);
			}
			;break;
			default: {
			}
