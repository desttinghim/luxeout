...
HX_STACK_FRAME("phoenix.Spatial","new",0x51d44037,"phoenix.Spatial.new","phoenix/Transform.hx",547,0x44da8ab2)
{
	...
	HX_STACK_LINE(567)
	{
		...
		if (((_m != null()))){
			...
			HX_STACK_LINE(567)
			this->floats = ::snow::platform::native::utils::Float32Array_obj::__new(_this->_float32array,null(),null());
		}
		HX_STACK_LINE(567)
		this->matrix;
	}
		...
		if (((bool(this->auto_decompose) || bool(_force)))){
			...
			HX_STACK_LINE(595)
			{
				...
				if (((_this->_transform == null()))){
					HX_STACK_LINE(595)
					_this->_transform = ::phoenix::MatrixTransform_obj::__new(_position,_quaternion,_scale);
				}
				else{
					HX_STACK_LINE(595)
