...
HX_STACK_ARG(options,"options")
{
	...
	this->centered = true;
	HX_STACK_LINE(26)
	this->set_uv(::phoenix::Rectangle_obj::__new(null(),null(),null(),null()));
	HX_STACK_LINE(28)
	if (((options == null()))){
		HX_STACK_LINE(29)
...
::phoenix::Vector Sprite_obj::set_size( ::phoenix::Vector _v){
	...
	if (((this->geometry_quad != null()))){
		HX_STACK_LINE(195)
		this->geometry_quad->resize(::phoenix::Vector_obj::__new(_v->x,_v->y,null(),null()));
		HX_STACK_LINE(198)
		if ((!(this->_has_custom_origin))){
			HX_STACK_LINE(199)
			if ((this->centered)){
				struct _Function_4_1{
					inline static ::phoenix::Vector Block( ::phoenix::Vector &_v){
						HX_STACK_FRAME("*","closure",0x5bdab937,"*.closure","luxe/Sprite.hx",201,0xf10014e0)
						{
							HX_STACK_LINE(201)
				...
				HX_STACK_LINE(201)
				this->set_origin(_Function_4_1::Block(_v));
			}
		}
	}
	HX_STACK_LINE(208)
...
bool Sprite_obj::set_centered( bool _c){
	...
	if (((this->size != null()))){
		...
		if ((_c)){
			HX_STACK_LINE(219)
			this->set_origin(::phoenix::Vector_obj::__new((Float(this->size->x) / Float((int)2)),(Float(this->size->y) / Float((int)2)),null(),null()));
		}
		else{
			HX_STACK_LINE(221)
			this->set_origin(::phoenix::Vector_obj::__new(null(),null(),null(),null()));
		}
	}
	HX_STACK_LINE(225)
