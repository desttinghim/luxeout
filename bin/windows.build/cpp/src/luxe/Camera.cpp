...
HX_STACK_ARG(options,"options")
{
	...
	this->shaking = false;
	HX_STACK_LINE(70)
	this->_size_factor = ::phoenix::Vector_obj::__new(null(),null(),null(),null());
	HX_STACK_LINE(71)
	this->_rotation_radian = ::phoenix::Vector_obj::__new(null(),null(),null(),null());
	HX_STACK_LINE(72)
	this->_rotation_cache = ::phoenix::Quaternion_obj::__new(null(),null(),null(),null());
	HX_STACK_LINE(73)
	...
	if (((options->__Field(HX_CSTRING("view"),true) == null()))){
		HX_STACK_LINE(94)
		this->view = ::phoenix::Camera_obj::__new(options);
	}
	else{
		HX_STACK_LINE(94)
		this->view = options->__Field(HX_CSTRING("view"),true);
	}
	HX_STACK_LINE(97)
...
::phoenix::Vector Camera_obj::set_center( ::phoenix::Vector _c){
	...
	HX_STACK_LINE(115)
	this->set_pos(::phoenix::Vector_obj::__new((_c->x - (Float(_g->w) / Float((int)2))),(_c->y - (Float(this->get_viewport()->h) / Float((int)2))),null(),null()));
	HX_STACK_LINE(116)
...
Float Camera_obj::set_zoom( Float _z){
	...
	if (((this->get_size() != null()))){
		...
		{
			HX_STACK_LINE(139)
			::phoenix::Vector _g = this->view->get_scale();		HX_STACK_VAR(_g,"_g");
			HX_STACK_LINE(139)
			{
				HX_STACK_LINE(139)
				Float _x = (_g->x * ((Float((int)1) / Float(this->_size_factor->x))));		HX_STACK_VAR(_x,"_x");
				HX_STACK_LINE(139)
				_g->x = _x;
				HX_STACK_LINE(139)
				if ((_g->_construct)){
		...
		HX_STACK_LINE(140)
		{
			...
			{
				HX_STACK_LINE(140)
				Float _y = (_g->y * ((Float((int)1) / Float(this->_size_factor->y))));		HX_STACK_VAR(_y,"_y");
				HX_STACK_LINE(140)
				_g->y = _y;
				HX_STACK_LINE(140)
				if ((_g->_construct)){
...
::phoenix::Vector Camera_obj::set_size( ::phoenix::Vector _size){
	...
	if (((_size == null()))){
		...
		::phoenix::Rectangle _g = this->get_viewport();		HX_STACK_VAR(_g,"_g");
		HX_STACK_LINE(172)
		this->set_center(::phoenix::Vector_obj::__new((Float(_g->w) / Float((int)2)),(Float(this->get_viewport()->h) / Float((int)2)),null(),null()));
		HX_STACK_LINE(173)
		this->size = _size;
		HX_STACK_LINE(174)
		{
	...
	HX_STACK_LINE(208)
	{
		...
		HX_STACK_LINE(208)
		Float _x = (Float((int)1) / Float(((this->_size_factor->x * this->get_zoom()))));		HX_STACK_VAR(_x,"_x");
		HX_STACK_LINE(208)
		_this->x = _x;
		HX_STACK_LINE(208)
		if ((_this->_construct)){
	...
	HX_STACK_LINE(209)
	{
		...
		::phoenix::Vector _this = this->view->get_scale();		HX_STACK_VAR(_this,"_this");
		HX_STACK_LINE(209)
		Float _y = (Float((int)1) / Float(((this->_size_factor->y * this->get_zoom()))));		HX_STACK_VAR(_y,"_y");
		HX_STACK_LINE(209)
		_this->y = _y;
		HX_STACK_LINE(209)
		if ((_this->_construct)){
	...
	HX_STACK_LINE(212)
	this->set_center(::phoenix::Vector_obj::__new((Float(_size->x) / Float((int)2)),(Float(_size->y) / Float((int)2)),null(),null()));
	HX_STACK_LINE(215)
	return this->size = ::phoenix::Vector_obj::__new(_size->x,_size->y,_size->z,_size->w);
}
	...
	HX_STACK_ARG(oncomplete,"oncomplete")
{
		...
		HX_STACK_LINE(223)
		::luxe::tween::Actuate_obj::tween(_g1,_t,_Function_1_1::Block(_p),true,null())->onComplete(oncomplete,null())->ease(::luxe::tween::easing::Quad_obj::get_easeInOut())->onUpdate( Dynamic(new _Function_1_2(_g)),null());
	}
return null();
}
...
Void Camera_obj::update( Float dt){
{
		...
		if ((this->shaking)){
			...
			HX_STACK_LINE(302)
			this->shake_vector = ::Luxe_obj::utils->geometry->random_point_in_unit_circle();
			HX_STACK_LINE(305)
			{
				HX_STACK_LINE(305)
				::phoenix::Vector _g = this->shake_vector;		HX_STACK_VAR(_g,"_g");
				HX_STACK_LINE(305)
				{
					HX_STACK_LINE(305)
					Float _x = (_g->x * this->shake_amount);		HX_STACK_VAR(_x,"_x");
					HX_STACK_LINE(305)
					_g->x = _x;
					HX_STACK_LINE(305)
					if ((_g->_construct)){
			...
			HX_STACK_LINE(306)
			{
				...
				{
					HX_STACK_LINE(306)
					Float _y = (_g->y * this->shake_amount);		HX_STACK_VAR(_y,"_y");
					HX_STACK_LINE(306)
					_g->y = _y;
					HX_STACK_LINE(306)
					if ((_g->_construct)){
			...
			HX_STACK_LINE(307)
			{
				...
				{
					HX_STACK_LINE(307)
					Float _z = (_g->z * this->shake_amount);		HX_STACK_VAR(_z,"_z");
					HX_STACK_LINE(307)
					_g->z = _z;
					HX_STACK_LINE(307)
					if ((_g->_construct)){
		...
		if (((bool((this->update_view_pos != null())) && bool((this->view != null()))))){
			struct _Function_2_1{
				inline static ::phoenix::Vector Block( ::luxe::Camera_obj *__this){
					HX_STACK_FRAME("*","closure",0x5bdab937,"*.closure","luxe/Camera.hx",327,0xd52a0e40)
					{
						HX_STACK_LINE(327)
						::phoenix::Vector _this = __this->update_view_pos;		HX_STACK_VAR(_this,"_this");
						HX_STACK_LINE(327)
			...
			HX_STACK_LINE(327)
			this->view->set_pos(_Function_2_1::Block(this));
			HX_STACK_LINE(328)
